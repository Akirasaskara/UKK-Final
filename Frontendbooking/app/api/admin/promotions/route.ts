...
import { normalizePaginatedEnvelope, normalizePromotionItem } from '@/lib/server/admin-compat';
...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  if (status && ['upcoming', 'active', 'expired'].includes(status)) {
  ...
  const path = qs ? `/api/admin/diskon?${qs}` : '/api/admin/diskon';
      ...
      return noStoreResponse(
        {
          status: false,
          statusCode: 502,
          message: 'Format respon promosi tidak sesuai kontrak.',
          error: 'Bad Gateway',
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    const upstreamResponse = await fetchUpstream('/api/admin/diskon', {
      method: 'POST',
      headers: {
        Accept: 'application/json',
