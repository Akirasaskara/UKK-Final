...
export async function POST(request: NextRequest): Promise<Response> {
    ...
    return noStoreResponse(
      {
        status: false,
        statusCode: 400,
        message: 'File foto profil member wajib diunggah dalam field file.',
        error: 'Bad Request',
        timestamp: new Date().toISOString(),
      },
      400,
    ...
    return noStoreResponse(
      {
        status: false,
        statusCode: 400,
        message: 'Ukuran foto melebihi batas maksimal 5MB.',
        error: 'Bad Request',
        timestamp: new Date().toISOString(),
      },
      400,
  ...
  try {
    const upstreamResponse = await fetchUpstream('/api/upload/members', {
      method: 'POST',
      headers: {
